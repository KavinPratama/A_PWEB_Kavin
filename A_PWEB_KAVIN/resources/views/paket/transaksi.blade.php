    ...
    <main class="max-w-7xl mx-auto px-4 pt-8">
        ...
        @if($orders->isEmpty())
            <div class="bg-[#2d2f36] rounded-xl border border-gray-700 p-12 text-center shadow-lg">
                ...
                <a href="{{ route('dashboard') }}" class="inline-block bg-[#cda06b] hover:bg-[#b58856] text-black font-extrabold py-3 px-6 rounded-lg shadow-lg transition">
                    Pesan Joki Sekarang
                </a>
            </div>
        @else
                ...
                <div class="bg-[#2d2f36] rounded-xl overflow-hidden shadow-lg border border-gray-700 transition hover:border-[#cda06b]">
                    <div class="bg-[#3a3d46] px-5 py-4 flex justify-between items-center border-b border-gray-700">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-bold text-white text-sm">{{ $order->invoice_number }}</span>
                        </div>
                        ...
                        @elseif($order->status == 'Proses Joki')
                        ...
                        <div class="flex justify-between items-start">
                            <div>
                                <h3 class="text-lg font-bold text-[#cda06b] mb-1">
                                    {{ ucwords(str_replace('_', ' ', $order->paket)) }}
                                </h3>
                                <p class="text-xs text-gray-400 flex items-center gap-1">
                                    <i class="fa-solid fa-star text-yellow-500"></i> Target: {{ $order->jumlah_bintang }} Bintang
                                </p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-gray-400 mb-1">Total Pembayaran</p>
                                <p class="font-bold text-white">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-[#1f2125] rounded-lg p-4 border border-gray-700 text-sm">
                            <div class="grid grid-cols-2 gap-y-3">
                                <div>
                                    ...
                                    <p class="text-gray-200 font-semibold truncate">{{ $order->nickname }}</p>
                                </div>
                                <div>
                                    <p class="text-gray-500 text-[11px] uppercase tracking-wider font-bold mb-1">Login Via</p>
                                    <p class="text-gray-200 font-semibold uppercase">{{ $order->login_via }}</p>
                                </div>
                                <div class="col-span-2 border-t border-gray-700 pt-3 mt-1">
                                    ...
                                    @if($order->worker_id)
                                        ...
                                        <p class="text-xs text-gray-400 mt-1">Estimasi Selesai: {{ $order->estimasi_hari ?? '-' }} Hari</p>
                                    @else
