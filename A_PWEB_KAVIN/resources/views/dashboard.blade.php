    ...
    <header class="bg-red-700 dark:bg-red-900 py-4 border-b border-red-800 sticky top-0 z-50 text-white">
        <div class="max-w-7xl mx-auto px-4 flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
                <span class="text-sm font-semibold">Halo, {{ Auth::user()->username ?? Auth::user()->name }}!</span>
    ...
    <main class="max-w-7xl mx-auto px-4 pt-8 pb-12">
        <div class="flex justify-between items-center mb-8">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('worker.index') }}" class="bg-purple-700 hover:bg-purple-600 text-white px-4 py-2 rounded-lg font-bold border border-purple-500 transition shadow-[0_0_15px_rgba(147,51,234,0.3)] flex items-center gap-2">
                ...
                <div class="bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300 px-4 py-2 rounded-lg font-bold border border-blue-300 dark:border-blue-700">
                    Total Pesanan Masuk: {{ isset($semua_order) ? $semua_order->count() : 0 }}
                </div>
            </div>
        </div>
            ...
            <div class="bg-white dark:bg-[#1f2125] rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse($ordersGendong as $order)
                            <tr onclick="window.location='{{ route('admin.order.show', $order->id) }}'" class="hover:bg-amber-50 dark:hover:bg-amber-500/10 cursor-pointer transition">
                                <td class="p-4">
                                    <span class="font-bold text-[#f59e0b] block">{{ $order->invoice_number }}</span>
                                    <span class="text-green-600 dark:text-green-400 font-semibold">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</span>
                                    <p class="text-[10px] text-gray-400 mt-1 uppercase">{{ $order->payment_method }}</p>
                                </td>
                                <td class="p-4">
                                    <span class="font-bold text-gray-900 dark:text-white uppercase">{{ str_replace('_', ' ', $order->paket) }}</span>
                                    <p class="text-yellow-500 font-semibold text-xs"><i class="fa-solid fa-star"></i> {{ $order->jumlah_bintang }} Win</p>
                                </td>
                                <td class="p-4 text-xs space-y-1">
                                    <p><span class="text-gray-400">Nick:</span> <span class="font-bold text-gray-900 dark:text-white">{{ $order->nickname }}</span></p>
                                    <p><span class="text-gray-400">ID Game:</span> <span class="text-cyan-600 dark:text-cyan-400">{{ $order->request_hero }}</span></p>
                                    <p><span class="text-gray-400">Jadwal/Role:</span> <span class="italic text-yellow-600 dark:text-yellow-300">{{ $order->catatan }}</span></p>
                                </td>
                                <td class="p-4 text-right">
                                    <span class="px-3 py-1 rounded-full text-xs font-bold
                                        @if($order->status == 'Pending') bg-yellow-100 dark:bg-yellow-900/50 text-yellow-700 dark:text-yellow-400
                                        @elseif($order->status == 'Proses Joki') bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-400
        ...
        <section class="mb-10">
            ...
            <div class="bg-white dark:bg-[#1f2125] rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse($ordersHero as $order)
                            <tr onclick="window.location='{{ route('admin.order.show', $order->id) }}'" class="hover:bg-cyan-50 dark:hover:bg-cyan-500/10 cursor-pointer transition">
                                <td class="p-4">
                                    <span class="font-bold text-[#5bcfe6] block">{{ $order->invoice_number }}</span>
                                    <span class="text-green-600 dark:text-green-400 font-semibold">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</span>
                                    <p class="text-[10px] text-gray-400 mt-1 uppercase">{{ $order->payment_method }}</p>
                                </td>
                                <td class="p-4">
                                ...
                                <td class="p-4 text-xs space-y-1">
                                    <p><span class="text-gray-400">Nick:</span> <span class="font-bold text-gray-900 dark:text-white">{{ $order->nickname }}</span> ({{ $order->login_via }})</p>
                                    <p><span class="text-gray-400">Email:</span> <span class="text-gray-700 dark:text-gray-300">{{ $order->email_game }}</span></p>
                                </td>
                                <td class="p-4 text-right">
                                    <span class="px-3 py-1 rounded-full text-xs font-bold
                                        @if($order->status == 'Pending') bg-yellow-100 dark:bg-yellow-900/50 text-yellow-700 dark:text-yellow-400
        ...
        <section class="mb-10">
            ...
            <div class="bg-white dark:bg-[#1f2125] rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse($ordersRank as $order)
                            <tr onclick="window.location='{{ route('admin.order.show', $order->id) }}'" class="hover:bg-green-50 dark:hover:bg-green-500/10 cursor-pointer transition">
                                <td class="p-4">
                                    <span class="font-bold text-[#cda06b] block">{{ $order->invoice_number }}</span>
                                    <span class="text-green-600 dark:text-green-400 font-semibold">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</span>
                                    <p class="text-[10px] text-gray-400 mt-1 uppercase">{{ $order->payment_method }}</p>
                                </td>
                                <td class="p-4">
                                    <span class="font-bold text-gray-900 dark:text-white uppercase">{{ str_replace('_', ' ', $order->paket) }}</span>
                                    <p class="text-yellow-500 font-semibold text-xs"><i class="fa-solid fa-star"></i> {{ $order->jumlah_bintang }} Bintang</p>
                                </td>
                                <td class="p-4 text-xs space-y-1">
                                    <p><span class="text-gray-400">Nick:</span> <span class="font-bold text-gray-900 dark:text-white">{{ $order->nickname }}</span> ({{ $order->login_via }})</p>
                                    <p><span class="text-gray-400">Email:</span> <span class="text-gray-700 dark:text-gray-300">{{ $order->email_game }}</span></p>
...
<body class="bg-gray-100 dark:bg-[#121316] text-gray-800 dark:text-gray-200 min-h-screen font-sans transition-colors duration-300">
    ...
    <div
        ...
        <img src="{{ asset('images/globalhero.png') }}"
    ...
    <div>
        ...
        <section class="popular-section mb-12">
            ...
            <div class="popular-grid grid grid-cols-1 md:grid-cols-3 gap-6">
                <a href="{{ route('paket.create', ['type' => 'rank']) }}" class="pop-card bg-[#2b1854] border border-[#5e20b3] hover:border-[#5bcfe6] rounded-xl overflow-hidden shadow-xl transition transform hover:-translate-y-1 block">
                    <div class="h-40 w-full bg-[#1c0f38] flex items-center justify-center p-4">
                        <img src="{{ asset('images/roaster.png') }}" alt="Joki Rank" class="h-full object-contain drop-shadow-2xl">
                    </div>
                ...
                <a href="{{ route('paket.create', ['type' => 'hero']) }}" class="pop-card bg-[#2b1854] border border-[#5e20b3] hover:border-[#5bcfe6] rounded-xl overflow-hidden shadow-xl transition transform hover:-translate-y-1 block">
                    <div class="h-40 w-full bg-[#1c0f38] flex items-center justify-center p-4">
                        <img src="{{ asset('images/globalhero.png') }}" alt="Joki Hero" class="h-full object-contain drop-shadow-2xl">
                    </div>
                    <div class="pop-info p-5 bg-gradient-to-t from-[#1c0f38] to-[#2b1854]">
                ...
                <a href="{{ route('paket.create', ['type' => 'gendong']) }}" class="pop-card bg-[#2b1854] border border-[#5e20b3] hover:border-[#5bcfe6] rounded-xl overflow-hidden shadow-xl transition transform hover:-translate-y-1 block">
                    <div class="h-40 w-full bg-[#1c0f38] flex items-center justify-center p-4">
                        <img src="{{ asset('images/epic.png') }}" alt="Joki Gendong" class="h-full object-contain drop-shadow-2xl">
                    </div>
                    <div class="pop-info p-5 bg-gradient-to-t from-[#1c0f38] to-[#2b1854]">
